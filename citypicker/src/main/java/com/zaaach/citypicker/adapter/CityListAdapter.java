...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private List<HistoryCity> lastSelection;
    ...
    public CityListAdapter(Context context, List<City> data, List<HotCity> hotData, int state,List<HistoryCity> lastSelection) {
        this.mData = data;
        this.mContext = context;
        this.mHotData = hotData;
        this.locateState = state;
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        ...
        if(holder instanceof HistoryHolder){
            ...
            final City data = mData.get(pos);
            if (data == null) return;
            GridListAdapter mAdapter = new GridListAdapter(mContext, lastSelection);
            mAdapter.setInnerListener(mInnerListener);
            ((HistoryHolder) holder).mRecyclerView.setAdapter(mAdapter);
        }
    }
...
//        fixme 暂时将定位移除
//        if (TextUtils.equals("定位", mData.get(position).getSection()))
//            return VIEW_TYPE_CURRENT;
        if(TextUtils.equals("历史", mData.get(position).getSection()))
            return VIEW_TYPE_HISTORY;
        if (TextUtils.equals("热门城市", mData.get(position).getSection()))
    ...
    public static class HistoryHolder extends BaseViewHolder{
        RecyclerView mRecyclerView;
